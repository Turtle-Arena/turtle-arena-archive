...
#ifdef RENDERLESS_MODELS
	jointMats = (float *)Hunk_AllocateTempMemory( header->num_joints * 2 * 3 * 4 * sizeof(float) );
#else
...
void R_AddIQMSurfaces( trRefEntity_t *ent ) {
	...
	for ( i = 0 ; i < data->num_surfaces ; i++ ) {
		...
		R_AddDrawSurf( &surface->surfaceType,
					surface->shader, 0 /*fogNum*/, qfalse, R_SortOrder(ent) );
#else
		R_AddDrawSurf( &surface->surfaceType,
